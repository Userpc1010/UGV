...
void dlio::OdomNode::getParams() {
  ...
  std::vector<float> baselink2imu_t{-0.011, -0.02329, 0.04412}, baselink2imu_R {1., 0., 0., 0., 1., 0., 0., 0., 1.};
...
//  ros::param::param<std::vector<float>>("~dlio/extrinsics/baselink2imu/R", baselink2imu_R, R_default);
  ...
  std::vector<float> baselink2lidar_t{0.0, 0.0, 0.0}, baselink2lidar_R{1., 0., 0., 0., 1., 0., 0., 0., 1.};
...
//  ros::param::param<std::vector<float>>("~dlio/extrinsics/baselink2lidar/R", baselink2lidar_R, R_default);
  ...
  this->verbose = true;  //отладочные сообщения
}
...
void dlio::OdomNode::publishPose() { //Update propogate stage
...
void dlio::OdomNode::publishToROS(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) { //Update new thread
...
void dlio::OdomNode::publishCloud(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) { //Update new thread too
...
void dlio::OdomNode::getScanFromROS(const CustomMsg pc) {
  ...
  original_scan_->header.frame_id = pc.lidar_id;
  original_scan_->header.stamp = pc.stamp;
  original_scan_->header.seq = pc.msg_seq;
  ...
  for (int i = 0; i < pc.point_num; i++) {
    ...
    p.timestamp = static_cast<uint32_t>(pc.points[i].offset_time);
  ...
  this->scan_header_stamp = pc.stamp;
...
void dlio::OdomNode::preprocessPoints() {
  ...
  if (this->deskew_) {
    ...
    std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> frames;
      frames = this->integrateImu(this->prev_scan_stamp, this->lidarPose.q, this->lidarPose.p,this->geo.prev_vel.cast<float>(), {this->scan_stamp});
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  if (this->sensor == dlio::SensorType::OUSTER) {
    ...
    point_time_neq = [](boost::range::index_value<PointType&, long> p1, boost::range::index_value<PointType&, long> p2){ return p1.value().timestamp != p2.value().timestamp; };
...
void dlio::OdomNode::callbackPointCloud(CustomMsg pc) {

  std::unique_lock<decltype(this->main_loop_running_mutex)> lock(main_loop_running_mutex);
  this->main_loop_running = true;
  lock.unlock();

  double then = timer.Time_milliseconds();

  if (this->first_scan_stamp == 0.) {
    this->first_scan_stamp = pc.stamp;
  }
  ...
  this->comp_times.push_back(timer.Time_milliseconds() - then);
...
void dlio::OdomNode::callbackImu(ImuConst imu_raw) {
...
void dlio::OdomNode::debug() {
  ...
  std::cout << "| Computation Time :: "
    << std::setfill(' ') << std::setw(6) << this->comp_times.back() << " ms    // Avg: "
    << std::setw(6) << avg_comp_time << " / Max: "
    << std::setw(6) << *std::max_element(this->comp_times.begin(), this->comp_times.end())
    << "     |" << std::endl;
...
double dlio::OdomNode::fromSec_pcl(double t)
{
