...
dlio::OdomNode::OdomNode()  {
  ...
  this->num_threads_ = omp_get_max_threads() / 2;

  //**====================================================================================================================================================================**//
  // Значит так, omp_get_max_threads() забирает себе все 20 ядер процессора и если ему их отдать поток от SetLivoxLidarImuDataCallback и SetLivoxLidarPointCloudCallBack
  // вероятно тоже замедляется, что приводит к их раннему срабатыванию (с интервалом 10-100 наносекунд) и падению этой ноды потому она ограничена omp_get_max_threads() / 2
  // аналогично, если в таком состоянии как оно сейчас есть у этой ноды загрузить все ядра ЦП с помощью sysbench --threads="$(nproc)" cpu run она начнёт виснуть но не упадёт
  // сразу из-за разных приоритетов процеса у SetLivoxLidarImuDataCallback он высокий у этой ноды низкий, но этот костыль не решил проблему как и не решил её костыль:
  // if(((timestamp - Last_upd) / 1000.0) < 2000 ) { Last_upd = timestamp; timestamp += 2000 - ((timestamp - Last_upd) / 1000.0); } что должен преписать время если
  // 2 измерения от imu пришли с интервалом 10-100 мс что вызовет сбой в расчётах этой ноды. Любопытно что в ROS эта нода под загрузкой ЦП до 100% работает нормально
  // и обратный вызов SetLivoxLidarImuDataCallback от лидара не зависает при включении sysbench на все ядра однако механизм работы этого явления установить не удалось т.к.
  // все мои программы зависают аналогичным образом и исключений я не знаю. Зависания увеличят время исполнения ноды с 3-2мс до 150-300мс что приведёт к дрейфу и сбоям!!!
  // Также в ROS эта нода ест меньше ресурсов ЦП или не правильно показывает расчёты но на 10 потоках она потребляет 10% загрузки а на 20 потоках 0.1% что довольно странно.
  //**====================================================================================================================================================================**//
...
//  std::vector<float> R_default{1., 0., 0., 0., 1., 0., 0., 0., 1.};
  ...
  std::vector<float> baselink2imu_t{-0.042382, -0.019593,   0.020312}, baselink2imu_R{1., 0., 0., 0., 1., 0., 0., 0., 1.};
...
//  ros::param::param<std::vector<float>>("~dlio/extrinsics/baselink2imu/R", baselink2imu_R, R_default);
  ...
  this->extrinsics.baselink2imu.R = Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(baselink2imu_R.data(), 3, 3);
  ...
  std::vector<float> baselink2lidar_t{0., 0., 0.}, baselink2lidar_R{1., 0., 0., 0., 1., 0., 0., 0., 1.};
...
//  ros::param::param<std::vector<float>>("~dlio/extrinsics/baselink2lidar/R", baselink2lidar_R, R_default);
  ...
  this->extrinsics.baselink2lidar.R = Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(baselink2lidar_R.data(), 3, 3);
...
//  ros::param::param<int>("~dlio/odom/imu/bufferSize", this->imu_buffer_size_, 2000);
  ...
  this->imu_buffer_size_ = 5000;

  std::vector<float> prior_accel_bias{0., 0., 0.};
  std::vector<float> prior_gyro_bias{0., 0., 0.};
...
//  ros::param::param<std::vector<float>>("~dlio/imu/intrinsics/gyro/bias", prior_gyro_bias, gyro_default);
  ...
  std::vector<float> imu_sm{1., 0., 0., 0., 1., 0., 0., 0., 1.};
...
//  ros::param::param<double>("~dlio/odom/gicp/maxCorrespondenceDistance", this->gicp_max_corr_dist_,std::sqrt(std::numeric_limits<double>::max()));
...
//  ros::param::param<double>("~dlio/odom/gicp/initLambdaFactor", this->gicp_init_lambda_factor_, 1e-9);
  ...
  this->gicp_k_correspondences_ = 16;
  this->gicp_max_corr_dist_ = 0.5;
  this->gicp_max_iter_ = 32;
  this->gicp_transformation_ep_ = 0.01;
  this->gicp_rotation_ep_ = 0.01;
...
//  ros::param::param<double>("~dlio/odom/geo/gbias_max", this->geo_gbias_max_, 1.0);
  ...
  this->geo_Kab_ = 2.25;
  this->geo_Kgb_ = 1.0;
  this->geo_abias_max_ = 5.0;
  this->geo_gbias_max_ = 0.5;
...
void dlio::OdomNode::publishPose() {
    ...
    this->odom.timestamp = this->imu_meas.stamp;

     this->odom.x_pos = this->state.p[0];
     this->odom.y_pos = this->state.p[1];
     this->odom.z_pos = this->state.p[2];

     this->odom.w = this->state.q.w();
     this->odom.x = this->state.q.x();
     this->odom.y = this->state.q.y();
     this->odom.z = this->state.q.z();

     this->odom.x_vel = this->state.v.lin.w[0];
     this->odom.y_vel = this->state.v.lin.w[1];
     this->odom.z_vel = this->state.v.lin.w[2];
...
void dlio::OdomNode::publishToROS(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
        ...
        unsigned long long size = this->submap_cloud->points.size();
        ...
        COLOR c; float min_map = 0.0f, max_map = 255.0f;
           ...
           {
               vertices_data_Map[i * 3] = 200.0f + this->submap_cloud->points[i].y * 100.0f;
               vertices_data_Map[i * 3 + 1] = -50.0f + this->submap_cloud->points[i].z  * 100.0f;
               vertices_data_Map[i * 3 + 2] = -200.0f + this->submap_cloud->points[i].x  * 100.0f;

               //qDebug()<<" intencity: "<<laserCloudWorld.intensity;

               c = GetColor(this->submap_cloud->points[i].intensity, min_map, max_map);
...
void dlio::OdomNode::getScanFromROS(const CustomMsg pc) {
  ...
  original_scan_->header.frame_id = this->lidar_frame;
    original_scan_->header.stamp = pc.header.stamp.toNSec() / 1000ull; //??????
    original_scan_->header.seq = pc.header.msg_seq;

    for (int i = 0; i < pc.point_num; i++) {
      PointType p;
      p.x = pc.points[i].x;
      p.y = pc.points[i].y;
      p.z = pc.points[i].z;
      p.intensity = pc.points[i].reflectivity;
      p.timestamp = static_cast<double>(pc.points[i].offset_time);
  ...
  //std::cout<<"                               original_scan_seq "<<std::setprecision(15)<<original_scan_->header.seq<<std::endl;
  //std::cout<<"                               original_scan_stamp "<<std::setprecision(15)<<original_scan_->header.stamp<<std::endl;
  //std::cout<<"                               original_offset_time "<<std::setprecision(15)<<original_scan_->points.at(0).timestamp<<std::endl;
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  if (frames.size() != timestamps.size()) {
    std::cout<<"Bad time sync between LiDAR and IMU!"<<std::endl;
...
void dlio::OdomNode::callbackPointCloud(const CustomMsg pc) {

  //std::cout<<" Deque_pc "<<std::setprecision(15)<<pc.header.stamp.toNSec() / 10000000.0<<std::endl;

  std::unique_lock<decltype(this->main_loop_running_mutex)> lock(main_loop_running_mutex);
  this->main_loop_running = true;
  lock.unlock();

  double then = static_cast<double>(timer.Time_microseconds()) / 1000000.0;
  ...
  if (this->current_scan->points.size() <= this->gicp_min_num_points_) {
    std::cout<<"Low number of points in the cloud!"<<std::endl;
    return;
  }
  ...
  this->comp_times.push_back(static_cast<double>(timer.Time_microseconds()) / 1000000.0f - then);
...
void dlio::OdomNode::callbackImu(const ImuConst imu_raw) {

  this->first_imu_received = true;

  std::shared_ptr<ImuConst> imu = this->transformImu( imu_raw );
  this->imu_stamp = imu->header.stamp;

  //std::cout<<" Deque_imu "<<std::setprecision(15)<<imu->header.stamp.toNSec() / 10000000.0<<std::endl;

  Eigen::Vector3f lin_accel;
  Eigen::Vector3f ang_vel;

  // Get IMU samples
  ang_vel[0] = imu->angular_velocity[0];
  ang_vel[1] = imu->angular_velocity[1];
  ang_vel[2] = imu->angular_velocity[2];

  lin_accel[0] = imu->linear_acceleration[0];
  lin_accel[1] = imu->linear_acceleration[1];
  lin_accel[2] = imu->linear_acceleration[2];
  ...
  if (!this->imu_calibrated) {
    ...
    //std::cout<<" Loop "<<imu->header.stamp.toSec()<<std::endl;
...
COLOR dlio::OdomNode::GetColor(float v, float vmin, float vmax)
{
...
std::shared_ptr<ImuConst>  dlio::OdomNode::transformImu(const ImuConst imu_raw) {

  std::shared_ptr<ImuConst> imu = std::make_shared<ImuConst>(imu_raw);
  ...
  Eigen::Vector3f ang_vel(imu_raw.angular_velocity[0],
                          imu_raw.angular_velocity[1],
                          imu_raw.angular_velocity[2]);

  Eigen::Vector3f ang_vel_cg = this->extrinsics.baselink2imu.R * ang_vel;

  imu->angular_velocity[0] = ang_vel_cg[0];
  imu->angular_velocity[1] = ang_vel_cg[1];
  imu->angular_velocity[2] = ang_vel_cg[2];
  ...
  Eigen::Vector3f lin_accel(imu_raw.linear_acceleration[0],
                            imu_raw.linear_acceleration[1],
                            imu_raw.linear_acceleration[2]);
  ...
  imu->linear_acceleration[0] = lin_accel_cg[0];
  imu->linear_acceleration[1] = lin_accel_cg[1];
  imu->linear_acceleration[2] = lin_accel_cg[2];
