    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
  ...
  Lidar_T_wrt_IMU = Vector3d (0, 0, 0);
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  if (b_first_frame_)
  {
    ...
    //double imu_acc [3] = meas.imu.front().linear_acceleration;
    //double gyr_acc [3] = meas.imu.front().angular_velocity;
    mean_acc << meas.imu.front()->linear_acceleration[0], meas.imu.front()->linear_acceleration[1], meas.imu.front()->linear_acceleration[2];
    mean_gyr << meas.imu.front()->angular_velocity[0], meas.imu.front()->angular_velocity[1], meas.imu.front()->angular_velocity[2];
    first_lidar_time = meas.lidar_beg_time;
  }

  for (const auto &imu : meas.imu)
  {
    //double imu_acc [3] = imu.linear_acceleration;
    //double gyr_acc [3] = imu.angular_velocity;
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)   //loop time 18ms
  {
    auto &&head = *(it_imu);
    auto &&tail = *(it_imu + 1);
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) //loop time 97ms
  {
    auto head = it_kp - 1;
    auto tail = it_kp;
    ...
    for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --) //loop time 5ms ~ 20 cycles
    {
